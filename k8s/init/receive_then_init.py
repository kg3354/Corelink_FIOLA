...
async def check_inactivity():
    global current_session_frames, current_session_start_time
    while True:
        await asyncio.sleep(1)
        current_time = time()
        if current_session_start_time and (current_time - incoming_frames["last_update_time"] > INACTIVITY_TIMEOUT):
...
async def callback(data_bytes, streamID, header):
    global incoming_frames, current_session_frames, current_session_start_time
